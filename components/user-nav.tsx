...
export function UserNav() {
  ...
  const [hasValidRegistration, setHasValidRegistration] = useState<boolean>(true)
    ...
    <div className="flex items-center gap-4">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" className="relative h-8 w-8 rounded-full">
            <Avatar className="h-8 w-8">
              <AvatarImage src={user.avatar_url || "/placeholder.svg?height=32&width=32"} alt={user.full_name} />
        ...
        <DropdownMenuContent className="w-56" align="end" forceMount>
          ...
          <DropdownMenuSeparator />
          <DropdownMenuGroup>
            <DropdownMenuItem onClick={() => router.push(getDashboardLink())}>{t("dashboard")}</DropdownMenuItem>
            ...
            {user.role === "admin" ? (
              <>
                ...
                <DropdownMenuItem onClick={() => router.push("/admin/certifications/professional")}>
                  {t("certification")}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push("/admin/fellowships")}>
              ...
              <>
                <DropdownMenuItem onClick={() => router.push("/reviewer/applications")}>
                  {language === "en" ? "Applications" : "الطلبات"}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push("/reviewer/fellowship")}>
                  {language === "en" ? "Fellowship Applications" : "طلبات الزمالة"}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push("/reviewer/documents")}>
                  {language === "en" ? "Documents" : "المستندات"}
                </DropdownMenuItem>
              </>
            ) : user.role === "registrar" ? (
              <>
                <DropdownMenuItem onClick={() => router.push("/registrar/applications")}>
                  <ClipboardList className="mr-2 h-4 w-4" />
                  {language === "en" ? "Registration Applications" : "طلبات التسجيل"}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push("/registrar/renewals")}>
                  {language === "en" ? "Renewal Requests" : "طلبات التجديد"}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push("/registrar/reports")}>
              ...
              <>
                ...
                <DropdownMenuItem onClick={() => router.push("/qualifications")}>
                  {t("qualifications")}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push("/certification")}>{t("certification")}</DropdownMenuItem>
              </>
            )}
          </DropdownMenuGroup>
          <DropdownMenuSeparator />
            ...
            <>
              <DropdownMenuGroup>
                <DropdownMenuItem onClick={handleFellowshipClick}>
                  ...
                  {language === "en" ? "Apply for Fellowship" : "التقدم للزمالة"}
                  {yearsOfExperience < 5 && <AlertCircle className="ml-auto h-4 w-4 text-yellow-500" />}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={handleInstitutionClick}>
                  <Building2 className="mr-2 h-4 w-4" />
                  {language === "en" ? "Register Institution" : "تسجيل مؤسسة"}
                  {!hasValidRegistration && <AlertCircle className="ml-auto h-4 w-4 text-yellow-500" />}
                </DropdownMenuItem>
              </DropdownMenuGroup>
              <DropdownMenuSeparator />
            </>
