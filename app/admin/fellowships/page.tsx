...
import { Award, Check, Edit, MoreHorizontal, Plus, Search, Trash2, Users } from "lucide-react"
...
const mockFellowships = [
  {
    ...
    descriptionEn: "Advanced fellowship for specialists in agricultural water management and conservation",
    descriptionAr: "زمالة متقدمة للمتخصصين في إدارة المياه الزراعية والحفاظ عليها",
    requirements: [
      {
        id: "req1",
      ...
      { id: "req3", nameEn: "Pass specialized examination", nameAr: "اجتياز الامتحان التخصصي" },
    ],
    renewalPeriod: 3,
    examRequired: true,
    isActive: true,
  ...
  },
  {
    ...
    descriptionEn: "Fellowship for experts in sustainable and organic farming practices",
    ...
    requirements: [
      ...
      { id: "req7", nameEn: "Pass specialized examination", nameAr: "اجتياز الامتحان التخصصي" },
    ],
    renewalPeriod: 3,
    examRequired: true,
    isActive: true,
  ...
  },
  {
    ...
    descriptionEn: "Fellowship for innovators in agricultural engineering technology and solutions",
    ...
    requirements: [
      ...
      { id: "req11", nameEn: "Pass specialized examination", nameAr: "اجتياز الامتحان التخصصي" },
    ],
    renewalPeriod: 3,
    examRequired: true,
    isActive: false,
...
export default function AdminFellowshipsPage() {
  ...
  const handleAddFellowship = () => {
    ...
    toast({
      ...
      description: language === "en" ? "The fellowship has been added successfully" : "تمت إضافة الزمالة بنجاح",
    })
  }
  ...
  const handleEditFellowship = () => {
    ...
    toast({
      ...
      description: language === "en" ? "The fellowship has been updated successfully" : "تم تحديث الزمالة بنجاح",
    })
  }

  const handleDeleteFellowship = () => {
    const updatedFellowships = fellowships.filter((fellowship) => fellowship.id !== currentFellowship.id)
    ...
    toast({
      ...
      description: language === "en" ? "The fellowship has been deleted successfully" : "تم حذف الزمالة بنجاح",
    })
  }

  const handleToggleStatus = (id: string) => {
    const updatedFellowships = fellowships.map((fellowship) =>
      fellowship.id === id ? { ...fellowship, isActive: !fellowship.isActive } : fellowship,
    )
    ...
    toast({
      ...
      description: language === "en" ? "The fellowship status has been updated" : "تم تحديث حالة الزمالة",
    })
  }
  ...
  const handleRemoveRequirement = (reqId: string) => {
    const updatedRequirements = currentFellowship.requirements.filter((req: any) => req.id !== reqId)
  ...
  return (
    <div className="container py-10">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold mb-2">{language === "en" ? "Fellowship Management" : "إدارة الزمالات"}</h1>
      ...
      <Card className="mb-6">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>{language === "en" ? "Fellowships" : "الزمالات"}</CardTitle>
            <div className="flex items-center gap-4">
              <div className="relative w-64">
                <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
                <Input
                  placeholder={language === "en" ? "Search fellowships..." : "البحث عن الزمالات..."}
                  className="pl-8"
                  value={searchQuery}
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-[400px]">
                <TabsList className="grid w-full grid-cols-3">
                  ...
                  <TabsTrigger value="active">{language === "en" ? "Active" : "نشط"}</TabsTrigger>
                  <TabsTrigger value="inactive">{language === "en" ? "Inactive" : "غير نشط"}</TabsTrigger>
                </TabsList>
              </Tabs>
            </div>
          </div>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>{language === "en" ? "Renewal Period" : "فترة التجديد"}</TableHead>
                <TableHead>{language === "en" ? "Fellows" : "الزملاء"}</TableHead>
              ...
              {filteredFellowships.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-6 text-muted-foreground">
                    {language === "en" ? "No fellowships found" : "لم يتم العثور على زمالات"}
                  </TableCell>
                </TableRow>
              ) : (
                  ...
                  <TableRow key={fellowship.id}>
                    <TableCell className="font-medium">
                      <div className="flex items-center gap-2">
                        ...
                        {language === "en" ? fellowship.nameEn : fellowship.nameAr}
                      </div>
                    </TableCell>
                    <TableCell className="max-w-xs truncate">
                      {language === "en" ? fellowship.descriptionEn : fellowship.descriptionAr}
                    </TableCell>
                    <TableCell>{fellowship.requirements.length}</TableCell>
                    <TableCell>
                      {fellowship.renewalPeriod} {language === "en" ? "years" : "سنوات"}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => openEditDialog(fellowship)}>
                            <Edit className="mr-2 h-4 w-4" />
                            {language === "en" ? "Edit" : "تعديل"}
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleToggleStatus(fellowship.id)}>
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{language === "en" ? "Add New Fellowship" : "إضافة زمالة جديدة"}</DialogTitle>
            <DialogDescription>
              {language === "en"
              ...
              <div className="grid grid-cols-1 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setCurrentFellowship({ ...currentFellowship, nameEn: e.target.value })}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setCurrentFellowship({ ...currentFellowship, nameAr: e.target.value })}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e) => setCurrentFellowship({ ...currentFellowship, descriptionEn: e.target.value })}
                    rows={3}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e) => setCurrentFellowship({ ...currentFellowship, descriptionAr: e.target.value })}
                    rows={3}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setCurrentFellowship({ ...currentFellowship, renewalPeriod: Number.parseInt(value) })
                    }
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="1">{language === "en" ? "1 year" : "1 سنة"}</SelectItem>
                      <SelectItem value="2">{language === "en" ? "2 years" : "2 سنة"}</SelectItem>
                      <SelectItem value="3">{language === "en" ? "3 years" : "3 سنوات"}</SelectItem>
                      <SelectItem value="5">{language === "en" ? "5 years" : "5 سنوات"}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="flex items-center space-x-2 rtl:space-x-reverse">
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setCurrentFellowship({ ...currentFellowship, examRequired: !!checked })
                    }
                  />
                  <Label htmlFor="examRequired">
                    {language === "en" ? "Specialized Exam Required" : "مطلوب امتحان متخصص"}
                  </Label>
                </div>
                <div className="flex items-center space-x-2 rtl:space-x-reverse">
                  <Switch
                    ...
                    onCheckedChange={(checked) => setCurrentFellowship({ ...currentFellowship, isActive: !!checked })}
                  />
                  <Label htmlFor="isActive">{language === "en" ? "Active Fellowship" : "زمالة نشطة"}</Label>
                </div>
              </div>
            </TabsContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddFellowship} className="bg-gradient-green hover:opacity-90">
              {language === "en" ? "Add Fellowship" : "إضافة زمالة"}
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{language === "en" ? "Edit Fellowship" : "تعديل الزمالة"}</DialogTitle>
            <DialogDescription>
              {language === "en" ? "Update the fellowship details and requirements." : "تحديث تفاصيل ومتطلبات الزمالة."}
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div className="flex items-center space-x-2 rtl:space-x-reverse">
                  <Switch
                    ...
                    onCheckedChange={(checked) => setCurrentFellowship({ ...currentFellowship, examRequired: checked })}
                  />
                ...
                <div className="flex items-center space-x-2 rtl:space-x-reverse">
                  <Switch
                    ...
                    onCheckedChange={(checked) => setCurrentFellowship({ ...currentFellowship, isActive: checked })}
                  />
                  <Label htmlFor="edit-isActive">{language === "en" ? "Active Fellowship" : "زمالة نشطة"}</Label>
                </div>
              </div>
            </TabsContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleEditFellowship} className="bg-gradient-green hover:opacity-90">
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{language === "en" ? "Delete Fellowship" : "حذف الزمالة"}</DialogTitle>
            <DialogDescription>
              {language === "en"
