...
import { Save, RefreshCw, Globe, Bell, Shield, Users, Mail, Loader2 } from "lucide-react"
...
export default function SystemSettingsPage() {
  ...
  // General Settings
  const [systemName, setSystemName] = useState("Agricultural Engineers Registration System")
  const [systemNameAr, setSystemNameAr] = useState("نظام تسجيل المهندسين الزراعيين")
  ...
  const [applicationStatusEmailEnabled, setApplicationStatusEmailEnabled] = useState(true)

  // Security Settings
  const [passwordMinLength, setPasswordMinLength] = useState(8)
  const [passwordRequireSpecialChar, setPasswordRequireSpecialChar] = useState(true)
  ...
  useEffect(() => {
    if (!authLoading) {
      if (!user) {
        router.push("/login")
      } else if (user.role !== "admin" && user.role !== "system-admin") {
  ...
  const handleSaveSettings = () => {
    ...
    toast({
      ...
      description:
        t("language") === "en" ? "System settings have been updated successfully" : "تم تحديث إعدادات النظام بنجاح",
    })
  }
  ...
  const handleResetSettings = () => {
    ...
    toast({
      title: t("language") === "en" ? "Settings Reset" : "تم إعادة تعيين الإعدادات",
      description:
        t("language") === "en"
  ...
  if (isLoading || authLoading) {
    return (
      <div className="container flex flex-col items-center justify-center min-h-screen">
        ...
        <p>{t("language") === "en" ? "Loading settings..." : "جاري تحميل الإعدادات..."}</p>
      </div>
    )
  }
  ...
  return (
    <div className="container py-10">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold mb-2">{t("language") === "en" ? "System Settings" : "إعدادات النظام"}</h1>
          <p className="text-muted-foreground">
            {t("language") === "en" ? "Configure system settings and preferences" : "تكوين إعدادات وتفضيلات النظام"}
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={handleResetSettings}>
            <RefreshCw className="mr-2 h-4 w-4" />
            {t("language") === "en" ? "Reset to Default" : "إعادة التعيين للافتراضي"}
          </Button>
          <Button onClick={handleSaveSettings} className="bg-gradient-green hover:opacity-90">
      ...
      <Tabs defaultValue="general">
        ...
        <TabsContent value="general">
          <Card className="bg-card">
            <CardHeader>
              <CardTitle>{t("language") === "en" ? "General Settings" : "الإعدادات العامة"}</CardTitle>
              <CardDescription>
                {t("language") === "en" ? "Configure basic system settings" : "تكوين إعدادات النظام الأساسية"}
              </CardDescription>
            </CardHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label htmlFor="systemName">
                    {t("language") === "en" ? "System Name (English)" : "اسم النظام (بالإنجليزية)"}
                  </Label>
                  <Input id="systemName" value={systemName} onChange={(e) => setSystemName(e.target.value)} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="systemNameAr">
                    {t("language") === "en" ? "System Name (Arabic)" : "اسم النظام (بالعربية)"}
                  </Label>
                  <Input id="systemNameAr" value={systemNameAr} onChange={(e) => setSystemNameAr(e.target.value)} />
                </div>
              </div>
              <div className="space-y-2">
                <Label htmlFor="defaultLanguage">
                  {t("language") === "en" ? "Default Language" : "اللغة الافتراضية"}
                </Label>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="maintenanceMode" className="block mb-1">
                    {t("language") === "en" ? "Maintenance Mode" : "وضع الصيانة"}
                  </Label>
        ...
        <TabsContent value="email">
          <Card className="bg-card">
            <CardHeader>
              <CardTitle>{t("language") === "en" ? "Email Settings" : "إعدادات البريد الإلكتروني"}</CardTitle>
              <CardDescription>
                {t("language") === "en"
            ...
            <CardContent className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="emailSender">
                  {t("language") === "en" ? "Email Sender Address" : "عنوان مرسل البريد الإلكتروني"}
                </Label>
                <Input
                  id="emailSender"
                  type="email"
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="emailNotifications" className="block mb-1">
                    {t("language") === "en" ? "Email Notifications" : "إشعارات البريد الإلكتروني"}
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    {t("language") === "en"
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {t("language") === "en" ? "Send welcome email to new users" : "إرسال بريد ترحيبي للمستخدمين الجدد"}
                  </p>
                </div>
                <Switch
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="applicationStatusEmailEnabled" className="block mb-1">
                    {t("language") === "en" ? "Application Status Updates" : "تحديثات حالة الطلب"}
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    {t("language") === "en"
        ...
        <TabsContent value="security">
          <Card className="bg-card">
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label htmlFor="passwordMinLength">
                    {t("language") === "en" ? "Minimum Password Length" : "الحد الأدنى لطول كلمة المرور"}
                  </Label>
                  <Input
                ...
                <div className="space-y-2">
                  <Label htmlFor="sessionTimeout">
                    {t("language") === "en" ? "Session Timeout (minutes)" : "مهلة الجلسة (دقائق)"}
                  </Label>
                  <Input
                    id="sessionTimeout"
                    type="number"
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="passwordRequireSpecialChar" className="block mb-1">
                    {t("language") === "en" ? "Require Special Characters" : "تتطلب أحرف خاصة"}
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    {t("language") === "en"
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="passwordRequireNumbers" className="block mb-1">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="twoFactorAuth" className="block mb-1">
                    {t("language") === "en" ? "Two-Factor Authentication" : "المصادقة الثنائية"}
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    {t("language") === "en"
        ...
        <TabsContent value="users">
          <Card className="bg-card">
            <CardHeader>
              <CardTitle>{t("language") === "en" ? "User Settings" : "إعدادات المستخدمين"}</CardTitle>
              <CardDescription>
                {t("language") === "en"
                  ? "Configure user registration and account settings"
                  : "تكوين إعدادات تسجيل ال��ستخدمين والحسابات"}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="allowSelfRegistration" className="block mb-1">
                    {t("language") === "en" ? "Allow Self Registration" : "السماح بالتسجيل الذاتي"}
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    {t("language") === "en"
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="requireEmailVerification" className="block mb-1">
                    {t("language") === "en" ? "Require Email Verification" : "تتطلب التحقق من البريد الإلكتروني"}
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    {t("language") === "en"
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="autoApproveUsers" className="block mb-1">
                    {t("language") === "en" ? "Auto-Approve New Users" : "الموافقة التلقائية على المستخدمين الجدد"}
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    {t("language") === "en"
              ...
              <div className="space-y-2">
                <Label htmlFor="maxLoginAttempts">
                  {t("language") === "en" ? "Maximum Login Attempts" : "الحد الأقصى لمحاولات تسجيل الدخول"}
                </Label>
                <Input
                  id="maxLoginAttempts"
                  type="number"
        ...
        <TabsContent value="notifications">
          <Card className="bg-card">
            <CardHeader>
              ...
              <CardDescription>
                {t("language") === "en" ? "Configure system notifications and alerts" : "تكوين إشعارات وتنبيهات النظام"}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="notifyNewApplications" className="block mb-1">
                    {t("language") === "en" ? "New Application Notifications" : "إشعارات الطلبات الجديدة"}
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    {t("language") === "en"
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="notifyApplicationStatus" className="block mb-1">
                    {t("language") === "en" ? "Application Status Notifications" : "إشعارات حالة الطلب"}
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    {t("language") === "en"
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="notifySystemUpdates" className="block mb-1">
                    {t("language") === "en" ? "System Update Notifications" : "إشعارات تحديثات النظام"}
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    {t("language") === "en"
            ...
            <CardFooter>
              <Button onClick={handleSaveSettings} className="bg-gradient-green hover:opacity-90 w-full">
                <Save className="mr-2 h-4 w-4" />
                {t("language") === "en" ? "Save Notification Settings" : "حفظ إعدادات الإشعارات"}
              </Button>
            </CardFooter>
          </Card>
        </TabsContent>
