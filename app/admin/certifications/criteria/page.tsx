...
export default function CertificationCriteriaPage() {
  ...
  const [criteria, setCriteria] = useState<CertificationCriteria[]>([
    ...
    },
    {
      ...
      category: "training",
      weight: 15,
      description: "Weight given to relevant training courses and certifications",
    ...
    },
    {
      ...
      category: "projects",
      weight: 15,
      description: "Weight given to significant professional projects completed",
      descriptionAr: "الوزن المعطى للمشاريع المهنية المهمة المكتملة",
    },
  ])
  const [dialogOpen, setDialogOpen] = useState(false)
  const [editingCriteria, setEditingCriteria] = useState<CertificationCriteria | null>(null)
  ...
  const handleSaveCriteria = () => {
    if (editingCriteria) {
      ...
      setCriteria(criteria.map((c) => (c.id === editingCriteria.id ? formData : c)))
      toast({
    ...
    return (
      <div className="container flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-lg">{t("language") === "en" ? "Loading..." : "جاري التحميل..."}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="container py-10">
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={handleAddCriteria} className="bg-gradient-green hover:opacity-90">
      ...
      <Card className="bg-card mb-6">
        <CardHeader>
          <CardTitle>{t("language") === "en" ? "Weight Distribution" : "توزيع الأوزان"}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium">
              {t("language") === "en" ? "Total Weight" : "إجمالي الوزن"}: {totalWeight}%
            </span>
            <span
              className={`text-sm font-medium ${
                totalWeight === 100 ? "text-green-500" : totalWeight < 100 ? "text-yellow-500" : "text-red-500"
              }`}
            >
              {totalWeight === 100
                ? t("language") === "en"
      ...
      <Card className="bg-card">
        <CardHeader>
          <CardTitle>{t("language") === "en" ? "Certification Criteria" : "معايير الشهادات"}</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>{t("language") === "en" ? "Criteria Name" : "اسم المعيار"}</TableHead>
                <TableHead>{t("language") === "en" ? "Category" : "الفئة"}</TableHead>
                <TableHead>{t("language") === "en" ? "Weight" : "الوزن"}</TableHead>
            ...
            <TableBody>
              {criteria.map((c) => (
                <TableRow key={c.id}>
                  <TableCell className="font-medium">{language === "en" ? c.name : c.nameAr}</TableCell>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      <Button variant="outline" size="sm" onClick={() => handleEditCriteria(c)}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="name">
                  {t("language") === "en" ? "Criteria Name (English)" : "اسم المعيار (بالإنجليزية)"}
                </Label>
                <Input
                  id="name"
                  name="name"
                  ...
                  placeholder={t("language") === "en" ? "e.g. Academic Degree" : "مثال: Academic Degree"}
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="nameAr">
                  {t("language") === "en" ? "Criteria Name (Arabic)" : "اسم المعيار (بالعربية)"}
                </Label>
                <Input
                  id="nameAr"
                  name="nameAr"
                  ...
                  onChange={handleChange}
                  placeholder={t("language") === "en" ? "e.g. الدرجة العلمية" : "مثال: الدرجة العلمية"}
                />
              </div>
            </div>
            <div className="space-y-2">
              <Label htmlFor="category">{t("language") === "en" ? "Category" : "الفئة"}</Label>
              <Select value={formData.category} onValueChange={(value) => handleSelectChange("category", value)}>
                <SelectTrigger id="category">
                  <SelectValue placeholder={t("language") === "en" ? "Select category" : "اختر الفئة"} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="education">{t("language") === "en" ? "Education" : "التعليم"}</SelectItem>
                  <SelectItem value="experience">{t("language") === "en" ? "Experience" : "الخبرة"}</SelectItem>
                  <SelectItem value="training">{t("language") === "en" ? "Training" : "التدريب"}</SelectItem>
                  <SelectItem value="research">{t("language") === "en" ? "Research" : "البحث"}</SelectItem>
                  <SelectItem value="projects">{t("language") === "en" ? "Projects" : "المشاريع"}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              <div className="flex justify-between">
                <Label htmlFor="weight">{t("language") === "en" ? "Weight (%)" : "الوزن (%)"}</Label>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveCriteria} className="bg-gradient-green hover:opacity-90">
